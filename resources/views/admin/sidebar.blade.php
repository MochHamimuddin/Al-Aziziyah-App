  <!-- ======= Sidebar ======= -->
  <aside id="sidebar" class="sidebar">

    <ul class="sidebar-nav" id="sidebar-nav">

      <li class="nav-item">
        <a class="nav-link collapsed " href="/dashboard">
          <i class="bi bi-grid"></i>
          <span>Dashboard</span>
        </a>
      </li><!-- End Dashboard Nav -->
      <li class="nav-item">
        <a class="nav-link collapsed" href="/datasantri">
          <i class="bi bi-person"></i>
          <span>Data Santri</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link collapsed" href="/datakelas">
          <i class="bi bi-display"></i>
          <span>Data Kelas</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link collapsed" href="/datakamar">
          <i class="bi bi-door-open"></i>
          <span>Data Kamar</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link collapsed" href="/datakesehatan">
          <i class="bi bi-patch-plus"></i>
          <span>Data Kesehatan</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link collapsed" data-bs-target="#components-nav" data-bs-toggle="collapse" href="#">
          <i class="bi bi-envelope"></i><span>Data Perizinan</span><i class="bi bi-chevron-down ms-auto"></i>
        </a>
        <ul id="components-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
          <li>
            <a href="/izinkeluar">
              <i class="bi bi-circle"></i><span>Izin Keluar</span>
            </a>
          </li>
          <li>
            <a href="/izinsakit">
              <i class="bi bi-circle"></i><span>Izin Sakit</span>
            </a>
          </li>
          <li>
            <a href="/izinpulang">
              <i class="bi bi-circle"></i><span>Izin Pulang</span>
            </a>
          </li>
        </ul>
      </li><!-- End Components Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" data-bs-target="#forms-nav" data-bs-toggle="collapse" href="#">
          <i class="bi bi-cash-stack"></i><span>Data Pembayaran</span><i class="bi bi-chevron-down ms-auto"></i>
        </a>
        <ul id="forms-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
          <li>
            <a href="/datapembayaran">
              <i class="bi bi-circle"></i><span>Data Pembayaran</span>
            </a>
          </li>
          <li>
            <a href="/riwayatpembayaran">
              <i class="bi bi-circle"></i><span>Riwayat Pembayaran</span>
            </a>
          </li>
        </ul>
      </li><!-- End Forms Nav -->

      <li class="nav-item">
        <a class="nav-link collapsed" href="/">
          <i class="bi bi-box-arrow-in-left"></i>
          <span>Log out</span>
        </a>
      </li><!-- End Login Page Nav -->
    </ul>

  </aside><!-- End Sidebar-->